...
export const PieChartTotalAnomaliesComparation: React.FC = ({height, rangeDatetime}) => {
  const [digitalModels, setDigitalModes] = useState([]);
  const [totalAnomaliesListData, setTotalAnomaliesListData] = useState([]);
  ...
  const retrieveAllDataLogsSamples = async () => {
    ...
    const promises = digitalModels.map(async (digital_model) => {
      ...
      const logsAnomalies = await retrieveDataLogsAnomalies(digital_model.id);
      if (logsAnomalies) {
        dictDigitalModelsLogsSamples[digital_model.id] = {total: logsAnomalies?.length || 0, name: digital_model?.name};
      }
    });
    await Promise.all(promises);
    setTotalAnomaliesListData(Object.values(dictDigitalModelsLogsSamples));
  };

  const retrieveDataLogsAnomalies = async (idDigitalModel) => {
    try {
  ...
  const getDataSeries = () => {
    const seriesData = [];
    if (totalAnomaliesListData) {
      totalAnomaliesListData.forEach((totalData) => {
        let item = {
  ...
  const getDataOption = () => {
    const colors = ['#cc6966', '#9f6fc2', '#d275ac', '#c2c079', '#dc8c66', '#5aaeb6'];
